<header class="bg-white shadow sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
        ...
        <div class="hidden md:flex items-center space-x-4">
            {{-- Cart Icon --}}
            <a href="{{ route('cart.index') }}" class="relative ml-4 text-gray-600 hover:text-blue-600 transition">
    ...
    {{-- Mobile Menu --}}
    <div id="mobile-menu" class="hidden md:hidden px-4 pb-4 space-y-1 text-gray-700 bg-white border-t">
        @foreach ($navLinks as $link)
        ...
        <div class="border-t pt-4 mt-2 space-y-2">
            @if (session('customer'))
                ...
                <form action="{{ route('customer.logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="block w-full text-left py-2 px-3 text-red-600 hover:bg-red-100 rounded">Logout</button>
                </form>
            @else
