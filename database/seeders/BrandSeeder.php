<?php

namespace Database\Seeders;

use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;

class BrandSeeder extends Seeder
{
    public function run(): void
    {

        DB::table('brands')->insert([
            [
                'name' => 'Samsung',
                'image_url' => 'data:image/png;base64,',
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'name' => 'Apple',
                'image_url' => 'https://yt3.googleusercontent.com/t4LKRr08hPIbHfUM6YRSbBY_VsEeMnEzlyOH8VUDny2gs7Lpf7vvt4pzuFVuVGmO6rw4YGcc2Ik=s176-c-k-c0x00ffffff-no-rj-mo',
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'name' => 'Xiaomi',
                'image_url' => 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS8Pfq44CzHA3XJsgt1G1e5kuyluRVHValhMQ&s',
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'name' => 'OPPO',
                'image_url' => 'https://cdn-icons-png.flaticon.com/512/5968/5968992.png',
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'name' => 'realme',
                'image_url' => 'https://www.logo.wine/a/logo/Oppo/Oppo-Logo.wine.svg',
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'name' => 'Vivo',
                'image_url' => 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRIHTog-ba9n0OgDoMDG9PXRns8-3qG6FvpAQ&s',
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'name' => 'Google',
                'image_url' => 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRdfv5_Zy38HWN8US65jtpvhg2ZwBnr9UEG3g&s',
                'created_at' => now(),
                'updated_at' => now(),
            ],
            [
                'name' => 'Huawei',
                'image_url' => 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQimEZYMLwskC8IvvAtSF07h15P1hNAPbwsFg&s',
                'created_at' => now(),
                'updated_at' => now(),
            ],
        ]);
    }
}
